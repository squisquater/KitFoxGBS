...
```
kfsuit.cost <- (101-kfsuit[[1]])*0.5
roads.cost <-  roads_clipped[[1]]*50

cost <- (kfsuit.cost + roads.cost)

## Reclassify the cost surface so there are no 0 values
# Define the reclassification matrix
# The first column is the current value, and the second column is the new value
#reclass_matrix <- matrix(c(0, 1), ncol = 2, byrow = TRUE)

# Apply the reclassification
#cost_reclassified <- classify(cost, rcl = reclass_matrix)

png("KFfull.CostSurface-20240722.png", width = 800, height = 600)
terra::plot(cost[[1]])
terra::points(points, pch=21, col="black", bg="white", cex=2)
dev.off()

writeRaster(cost, "KFfull_CostSurface_20240722.tif", overwrite=TRUE)
```

Create a conductance surface
```
conductance1 <- (101-cost[[1]])
...
```
### NEW 20240719
Load in the Source .tif file and plot it
```
...
savefig("KitFoxSourceModel.png")
```
Load in the Resistance .tif file and plot it
```
plot(Raster("/group/ctbrowngrp2/sophiepq/KitFoxGBS/Omniscape/kf_omniscape_20240722/KFfull_CostSurface_20240722.tif"), title = "KitFoxFullModel", xlabel = "Easting", ylabel = "Northing", size = (700,640))
savefig("KitFoxResistanceModel.png")
```

Load the array using Omniscape's internal read_raster() function 
```
land_cover_KF, wkt, transform = Omniscape.read_raster("/group/ctbrowngrp2/sophiepq/KitFoxGBS/Omniscape/kf_omniscape_20240722/KFfull_CostSurface_20240722.tif", Float64)
```
...
config = Dict{String, String}(
    ...
    "r_cutoff" => "90", # Only high quality pixels should be sources
...
savefig("kf_omniscape_output/KFflow_potential.png")
```
...
savefig("kf_omniscape_output/KFnormalized_current.png")
